...
class map {
	...
	public:
		...
		const map<TYPE>& operator=(const map<TYPE>& mapCopy);	// surcharge l'opérateur =
		TYPE* operator[](int pos)const;							// surcharge de l'opérateur []
};
// surchage de l'opérateur <<
template <class TYPE>
ostream& operator<<(ostream& output, const map<TYPE>& m) {
	m.print(output);
	return output;
}
...
// surchage de l'opérateur >>
template <class TYPE>
istream& operator>>(istream& input, map<TYPE>& m) {
	m.read(input);
	return input;
}
...
// copieur
template <class TYPE>
map<TYPE>::map(const map<TYPE>& mapCopy) {
	_line = mapCopy._line;
	_col = mapCopy._col;
...
template <class TYPE>
void map<TYPE>::print(ostream& output)const {
	for (int i = 0; i < _line; i++) {
		for (int j = 0; j < _col; j++) {
			output << *(*(_map + i) + j) << " ";
		}
...
template <class TYPE>
void map<TYPE>::read(istream& input) {
	for (int i = 0; i < _line; i++) {
		for (int j = 0; j < _col; j++) {
			input >> *(*(_map + i) + j);
		}
...
// surcharge de l'opérateur =
template <class TYPE>
const map<TYPE>& map<TYPE>::operator=(const map<TYPE>& mapCopy) {
